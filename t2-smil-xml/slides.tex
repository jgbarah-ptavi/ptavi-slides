...
\date[Sep 2016]{28 de septiembre de 2016}
...
\item Especificación de propósito general para crear lenguajes de marcado.
  \begin{itemize}
...
\item Típicamente se integran:
  \begin{itemize}
  \item audio/vídeo
  \item \emph{streaming} de audio/vídeo
  \item imágenes
  \item texto
  \item otros medios
  \end{itemize}
...
\item Proporciona mecanismos de sincronización para hacer coincidir, por ejemplo, una cierta frase hablada con la aparición de un cierto elemento gráfico.
\item Idea básica: nombrar los medios que componen la presentación (texto, imágenes, audio, vídeo) mediante URLs, y planificar su presentación en paralelo o en secuencia (y combinaciones).
\end{itemize}
...
\begin{frame}
\frametitle{Para finalizar... sí, hay gente que es realmente friqui}
...
\begin{itemize}
  ...
  \item Subconjunto de la notación literal de objetos de JavaScript
  \item Amplia adopción como alternativa a XML
  \item Mucho más sencillo escribir un analizador sintáctico (parser) de JSON 
\end{itemize}
...
Importar funcionalidad (módulo \texttt{json}):
\begin{verbatim}
import json
\end{verbatim}
